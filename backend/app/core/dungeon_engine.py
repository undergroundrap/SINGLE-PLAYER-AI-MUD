...
Each dungeon run has 3 rooms (trash → trash+elite → boss).
...
_ROOM_NAMES = [
    ...
    ["The Boss Chamber", "The Throne Room", "The Inner Sanctum"],
]
...
def _build_rooms(level: int, run_id: str) -> list[DungeonRoom]:
    mob_pool = _get_mob_pool(level)
    rooms = []
    for i, name_options in enumerate(_ROOM_NAMES):
        room_name = random.choice(name_options)
        mob_name  = random.choice(mob_pool)
        if i == 0:
            mobs = _make_mobs(mob_name, level, run_id, i, count=4)
        elif i == 1:
            mobs = _make_mobs(mob_name, level, run_id, i, count=3)
        else:  # boss room
            mobs = _make_mobs(mob_name, level, run_id, i, count=1, force_boss=True)
        # Ensure all mob IDs are unique within the run
...
def generate_run(player: Player, is_raid: bool = False) -> DungeonRun:
    ...
    return DungeonRun(
        ...
        rooms=_build_rooms(player.level, run_id),
...
def resolve_round(run: DungeonRun, player: Player) -> dict:
    ...
    if wiped:
        ...
        run.status = "cleared"
        round_log.append("★ DUNGEON CLEARED!")
    ...
    # ── 8. Keep rolling combat log (last 5 meaningful lines) ─────────────────
    run.combat_log = (run.combat_log + round_log)[-5:]

    # ── 9. Roll dungeon loot on boss death ───────────────────────────────────
    loot_items = []
    if run_cleared:
        boss = next((m for m in room.mobs if m.is_named), room.mobs[0] if room.mobs else None)
        if boss:
            # Guaranteed drop + bonus roll on named
            for _ in range(2 if boss.is_named else 1):
                item = _roll_loot(boss.level, boss.loot_table,
                                  char_class=player.char_class, zone_tier="dungeon")
                if item:
